...
(defn inject-into-eventmap
  [event lifecycle]
  (when-not (:lazy-seq/example-datasource event)
    (throw (ex-info ":lazy-seq/example-datasource not found - add it using a :before-task-start lifecycle"
  ...
  (let [pipeline (:onyx.core/pipeline event)] 
    {:core.async/pending-messages (:pending-messages pipeline) 
     ...
     :lazy-seq/example-datasource (:lazy-seq/example-datasource event)}))
...
(defrecord ExampleInput [max-pending batch-size batch-timeout pending-messages drained?]
  ...
  (read-batch [_ {:keys [lazy-seq/example-datasource] :as event}]
  ...
  (retry-segment 
    [_ {:keys [lazy-seq/example-datasource] :as event} message-id]
